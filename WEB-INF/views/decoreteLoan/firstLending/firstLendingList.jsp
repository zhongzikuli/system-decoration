...
<div class="mod_header">
    ...
    <form id="pagerForm" action="${ctx}/firstLending/firstLendingQuery.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
            <div class="col-sm-2">
               <%-- <c:if test="${paramMap.firstLendingOrderStatus == 16}">--%>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-5">
                            <div class="form-group">
                                ...
                                <div class="col-xs-8">
                                    <select class="form-control" id="search-style" name="firstLendingOrderStatus">
                                        <option value="0">全部</option>
                                        <option value="16" <c:if test="${paramMap.firstLendingOrderStatus ==16}">selected</c:if>>未放款</option>
                                        <option value="20" <c:if test="${paramMap.firstLendingOrderStatus == 20}">selected</c:if>>已放款</option>
                                        <option value="100" <c:if test="${paramMap.firstLendingOrderStatus == 100}">selected</c:if>>放款退回</option>
                                    </select>
                                </div>
                            </div>
            ...
            <table class="table table-hover table-height table-striped">
                ...
                <tr>
                   ...
                   <%-- </c:if>--%>
                    ...
                    <td>
                        <fmt:formatDate value="${item.firstVisitHandleTime}" pattern="yyyy-MM-dd HH:mm"/>
                    </td>
                    <td>
                        ...
                        <shiro:hasPermission name="firstLending:audit">
                            <c:if test="${item.orderStatus ==16}">
                                <a data-id="${item.id}" data-title="${item.realName}" data-url="${ctx}/cfBusinessOrderAccept/detail.action?id=${item.id}&active=playMoney"
                                   class="btn btn-primary btn-xs audit-detail"><i class="fa fa-edit"> 放款</i></a>
                            </c:if>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="firstLending:view">
                            <a data-id="${item.id}"  data-title="${item.realName}" data-href="${ctx}/dlBusinessHandle/detail.action?id=${item.id}&active=playMoney"
...
<script type="text/javascript" src="${ctx}/js/mine/decoreteLoan/firstLending/firstLendingList.js"></script>
</html>
