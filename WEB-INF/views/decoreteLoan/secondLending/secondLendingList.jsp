...
<div class="mod_header">
    <div id="hiddenForm">
        <input type="hidden" name="firstLendingOrderStatus" value="${paramMap.firstLendingOrderStatus}"/>
    ...
    <form id="pagerForm" action="${ctx}/secondLending/secondLendingQuery.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
            <div class="col-sm-2">
                <%--<c:if test="${paramMap.secondLendingOrderStatus == 24}">--%>
                    ...
                    <div class="row">
                            ...
                            <div class="form-group" id="date-time">
                                ...
                                <div class="col-xs-8">
                                    <div class="input-group">
                                        ...
                                        <span class="input-group-addon">到</span> <input type="text" class="form-control w-120" name="endDate" id="search-end-date" value="${paramMap.endDate}" />
                                    </div>
                                </div>
                            </div>
                            <div class="form-group" id="common-date-time">
                                <label class="col-xs-4 control-label">一次放款时间:</label>
                                <div class="col-xs-8">
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="commonStartDate" id="search-common-start-date" value="${paramMap.commonStartDate}" />
                                        <span class="input-group-addon">到</span> <input type="text" class="form-control" name="commonEndDate" id="search-common-end-date" value="${paramMap.commonEndDate}" />
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-sm-5">
                            <div class="form-group">
                                ...
                                <div class="col-xs-8">
                                    <select class="form-control" id="search-style" name="secondLendingOrderStatus">
                                        <option value="0">全部</option>
                                        <option value="24" <c:if test="${paramMap.secondLendingOrderStatus ==24}">selected</c:if>>未放款</option>
                                        <option value="28" <c:if test="${paramMap.secondLendingOrderStatus == 28}">selected</c:if>>已放款</option>
                                        <option value="200" <c:if test="${paramMap.secondLendingOrderStatus == 200}">selected</c:if>>放款退回</option>
                                    </select>
                                </div>
                            </div>
            ...
            <table class="table table-hover table-height table-striped">
                ...
                <tr>
                   ...
                   <%-- </c:if>--%>
                    ...
                    <td>
                        <fmt:formatDate value="${item.secondLendingHandleTime}" pattern="yyyy-MM-dd HH:mm"/>
                    </td>

                    <td class="btn-cel">
                        <shiro:hasPermission name="secondLending:audit">
                            <c:if test="${item.orderStatus == 24}">
                                <a data-id="${item.id}" data-title="${item.realName}" data-url="${ctx}/cfBusinessOrderAccept/detail.action?id=${item.id}&active=playMoney"
                                   class="btn btn-primary btn-xs audit-detail"><i class="fa fa-edit"></i>放款</a>
                            </c:if>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="secondLending:view">
                            <a data-id="${item.id}"  data-title="${item.realName}" data-href="${ctx}/dlBusinessHandle/detail.action?id=${item.id}"
...
<script type="text/javascript" src="${ctx}/js/mine/decoreteLoan/secondLending/secondLendingList.js"></script>
</html>
