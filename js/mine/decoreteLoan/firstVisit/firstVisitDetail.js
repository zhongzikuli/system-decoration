$(function () {
    ...
    var validater = new ValidateWin("#firstVisitDiv");
    //返回列表
    $("#firstVisitBackList").on("click", function () {
        window.location.href = ctx + "/firstVisit/firstVisitQuery.action";
    });
    //----------------------客户历史订单----------------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------------------------------------------------
    //更换审核
    $("#firstVisitChangeAuditBtn").on("click", function () {
    ...
    //其他按钮
    $("#firstVisitRefuseOrderBtn,#firstVisitBackApplyBtn,#firstVisitOrderApplyBtn,#firstVisitSaveBtn,#firstVisitAgreeBtn").on("click", function () {
        // 初始化审核表单校验
        ...
        entity.suggest = $("#firstVisitSuggest").val();
        if (orderStatus == -4) {//拒单
            entity.operatorTypeStr = "一次上门：拒单";
        } else if (orderStatus == -2) {
            entity.operatorTypeStr = "一次上门：退回申请";
        } else if (orderStatus == 8) {
            entity.operatorTypeStr = "一次上门：退回到银行审批";
        } else if (orderStatus == 12) {
            entity.operatorTypeStr = "一次上门：保存";
        } else if (orderStatus == 16) {
            entity.operatorTypeStr = "一次上门：同意";
        }
        var dataUrl = ctx + "/firstVisit/goFirstVisit.action?acceptId=" + acceptId;
        HQdecorate.jsonPost(
            ctx + "/firstVisit/updateFirstVisit.action",
