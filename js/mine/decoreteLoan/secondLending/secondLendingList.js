$(function () {
    ...
    $(".audit-detail").on("click", function () {
        ...
        HQdecorate.post(
            ...
            function (res) {
                if (res.error == 1) {
                    var url= ctx + "/secondLending/goSecondLending.action?acceptId=" + acceptId;
                    HQdecorate.openTabForParent(url, "-order-second-lending" + acceptId, "二次放款-" + orderTitle);
    ...
    $(".change-audit-btn").on("click", function () {
        ...
        if (ck.length == 0) {
            ...
            $(ck).each(function () {
                idArr.push($(this).val());
                if ($(this).attr("data-handle-status") != "24") {
                    flag = false;
                    return;
                }
            });
            if (!flag) {
                HQdecorate.alertDialog("已经二次放款的数据不能进行更换审核");
                return false;
            }
