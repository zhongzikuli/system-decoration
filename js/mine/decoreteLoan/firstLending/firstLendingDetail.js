$(function () {
    ...
    //------------------------------------------------------------------------------------------------------------------
    var validater = new ValidateWin("#firstLendingDiv");
    //返回列表
    $("#firstLendingBackList").on("click", function () {
        window.location.href = ctx + "/firstLending/firstLendingQuery.action";
    });
    $("#search-money").on("blur", function () {
        var val = $(this).val();
        var loanMoney = $("#search-loan-money").val();
        if (parseInt(val) > parseInt(loanMoney)) {
            HQdecorate.faildMsg("放款金额不能大于贷款金额,请重新输入");
            $("#search-money").val("");
        }
    })
    //更换审核
    $("#firstLendingChangeAuditBtn").on("click", function () {
    ...
    //其他按钮
    $("#firstLendingRefuseOrderBtn,#firstLendingBackApplyBtn,#firstLendingOrderApplyBtn,#firstLendingSaveBtn,#firstLendingAgreeBtn").on("click", function () {
        // 初始化审核表单校验
        ...
        HQdecorate.confirmDialog("确认" + text + "吗？", true, {}, function () {
            var vo = {};
            var money = $("#search-money").val();
            var loanMoney = $("#search-loan-money").val();
            if (parseInt(money) > parseInt(loanMoney)) {
                HQdecorate.faildMsg("放款金额不能大于贷款金额,请重新输入");
            }
            ...
            vo.acceptId = acceptId;
            vo.orderStatus = orderStatus;
            if(orderStatus == 16||orderStatus == 20){
                vo.money = money;
            }
            vo.suggest = $("#firstLendingSuggest").val();
            if (orderStatus == -4) {//拒单
                vo.operatorTypeStr = "一次放款：拒单";
            } else if (orderStatus == -2) {
                vo.operatorTypeStr = "一次放款：退回申请";
            } else if (orderStatus == 12) {
                vo.operatorTypeStr = "一次放款：退回到一次上门";
            } else if (orderStatus == 16) {
                vo.operatorTypeStr = "一次放款：保存";
            } else if (orderStatus == 20) {
                vo.operatorTypeStr = "一次放款：同意";
            }
            var dataUrl = ctx + "/firstLending/goFirstLending.action?acceptId=" + acceptId;
            HQdecorate.jsonPost(
                ctx + "/firstLending/updateFirstLending.action",
                JSON.stringify(vo),
                function (res) {
                    if (res.error == 1) {
                        HQdecorate.successMsg("操作成功", 1000, function () {
