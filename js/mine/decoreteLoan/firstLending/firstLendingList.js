$(function(){
    ...
    $(".audit-detail").on("click", function(){
        ...
        HQdecorate.post(
            ...
            function (res) {
                if (res.error == 1) {
                    var url =  ctx + "/firstLending/goFirstLending.action?acceptId=" + acceptId;
                    HQdecorate.openTabForParent(url, "-order-first-lending" + acceptId, "一次放款-" + orderTitle);
    ...
    $(".change-audit-btn").on("click", function(){
        ...
        if (ck.length == 0) {
            ...
            $(ck).each(function () {
                idArr.push($(this).val());
                if ($(this).attr("data-handle-status") != "16") {
                    flag = false;
                    return;
                }
            });
            if (!flag) {
                HQdecorate.alertDialog("已经一次放款的数据不能进行更换审核");
                return false;
            }
