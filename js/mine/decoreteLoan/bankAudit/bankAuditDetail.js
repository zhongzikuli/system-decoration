$(function () {
    ...
    var validater = new ValidateWin("#bankAuditDiv");
    //返回列表
    $("#bankAuditBackList").on("click", function () {
    ...
    //更换审核
    $("#bankAuditChangeAuditBtn").on("click", function () {
    ...
    //其他按钮
    $("#bankAuditRefuseOrderBtn,#bankAuditBackApplyBtn,#bankAuditOrderApplyBtn,#bankAuditSaveBtn,#bankAuditAgreeBtn").on("click", function () {
        // 初始化审核表单校验
        ...
        HQdecorate.confirmDialog("确认" + text + "吗？", true, {}, function () {
            ...
            entity.suggest = $("#bankAuditSuggest").val();
            if (orderStatus == -4) {//拒单
                entity.operatorTypeStr = "银行审批：拒单";
            } else if (orderStatus == -2) {
                entity.operatorTypeStr = "银行审批：退回申请";
            } else if (orderStatus == 4) {
                entity.operatorTypeStr = "银行审批：退回到银行受理";
            } else if (orderStatus == 8) {
                entity.operatorTypeStr = "银行审批：保存";
            } else if (orderStatus == 12) {
                entity.operatorTypeStr = "银行审批：同意";
            }
            var dataUrl = ctx + "/bankAudit/goBankAudit.action?acceptId=" + acceptId;
            HQdecorate.jsonPost(
                ctx + "/bankAudit/updateBankAudit.action",
