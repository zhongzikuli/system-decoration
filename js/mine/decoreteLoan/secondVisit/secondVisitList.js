$(function(){
    ...
    $(".audit-detail").on("click", function(){
        ...
        HQdecorate.post(
            ...
            function (res) {
                if (res.error == 1) {
                    var url  = ctx + "/secondVisit/goSecondVisit.action?acceptId=" + acceptId;
                    HQdecorate.openTabForParent(url, "-order-second-visit" + acceptId, "二次上门-" + orderTitle);
    ...
    $(".change-audit-btn").on("click", function(){
        ...
        if (ck.length == 0) {
            ...
            $(ck).each(function () {
                idArr.push($(this).val());
                if ($(this).attr("data-handle-status") != "20") {
                    flag = false;
                    return;
                }
            });
            if (!flag) {
                HQdecorate.alertDialog("已经二次上门的数据不能进行更换审核");
                return false;
            }
