$(function () {
    ...
    //------------------------------------------------------------------------------------------------------------------
    var validater = new ValidateWin("#secondVisitDiv");
    //返回列表
    $("#secondVisitBackList").on("click", function () {
        window.location.href = ctx + "/secondVisit/secondVisitQuery.action";
    });

    //更换审核
    $("#secondVisitChangeAuditBtn").on("click", function () {
    ...
    //其他按钮
    $("#secondVisitRefuseOrderBtn,#secondVisitBackApplyBtn,#secondVisitOrderApplyBtn,#secondVisitSaveBtn,#secondVisitAgreeBtn").on("click", function () {
        // 初始化审核表单校验
        ...
        entity.suggest = $("#secondVisitSuggest").val();
        if (orderStatus == -4) {//拒单
            entity.operatorTypeStr = "二次上门：拒单";
        } else if (orderStatus == -2) {
            entity.operatorTypeStr = "二次上门：退回申请";
        } else if (orderStatus == 16) {
            entity.operatorTypeStr = "二次上门：退回到一次放款";
        } else if (orderStatus == 20) {
            entity.operatorTypeStr = "二次上门：保存";
        } else if (orderStatus == 24) {
            entity.operatorTypeStr = "二次上门：同意";
        }
        var dataUrl = ctx + "/secondVisit/goSecondVisit.action?acceptId=" + acceptId;
        HQdecorate.jsonPost(
            ctx + "/secondVisit/updateSecondVisit.action",
